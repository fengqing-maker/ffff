...
UINT NEAR WM_EDIT_CHANGE                = ::RegisterWindowMessage(_T("WM_EDIT_CHANGE"));
UINT NEAR WM_MENU_COMBO_SELECTION       = ::RegisterWindowMessage(_T("WM_MENU_COMBO_SELECTION"));
UINT NEAR WM_XLISTCTRL_BUTTON_CLICKED   = ::RegisterWindowMessage(_T("WM_XLISTCTRL_BUTTON_CLICKED"));
/////////////////////////////////////////////////////////////////////////////
// CXListCtrl
...
BEGIN_MESSAGE_MAP(CXListCtrl, CListCtrl)
	...
	ON_WM_MOUSEWHEEL()
	ON_WM_VSCROLL()
	ON_COMMAND_RANGE(IDM_COMBOX_MENU,IDM_COMBOX_MENU+10,OnComboxMenu)
	//}}AFX_MSG_MAP
...
{
	m_bReadOnly = FALSE;
	m_strNoItemText = _T("There are no items to show in this view.");
#ifndef DO_NOT_INCLUDE_XCOMBOLIST
...
void CXListCtrl::OnCustomDraw(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	*pResult = CDRF_DODEFAULT;
	...
	else if (pLVCD->nmcd.dwDrawStage == (CDDS_ITEMPREPAINT | CDDS_SUBITEM))
	{
		...
		{
			if (GetItemState(nItem, LVIS_SELECTED) && m_nClickItem == nItem && m_nClickSubItem == nSubItem)
		...
		else if(pXLCD && pXLCD[nSubItem].bEdit)
		{
			if (GetItemState(nItem, LVIS_SELECTED))
				DrawEdit(nItem,nSubItem,pDC, crText, crBkgnd, rect, pXLCD);
			else
				DrawText(nItem, nSubItem, pDC, crText, crBkgnd, rect, pXLCD);
			
			//*pResult = CDRF_SKIPDEFAULT;	// We've painted everything.
		}
		else if(pXLCD[nSubItem].bEditAndCombox)
		{
			
			rcEdit =CRect(rect.left+pXLCD[nSubItem].nWidthOfCombox+1,rect.top,rect.right,rect.bottom);
			if(GetItemState(nItem, LVIS_SELECTED))
			{
				DrawEdit(nItem,nSubItem,pDC, crText, crBkgnd, rcEdit, pXLCD);
				//*pResult = CDRF_SKIPDEFAULT;	// We've painted everything.
			}
		}
		else if(pXLCD && pXLCD[nSubItem].bButton)
		{
			DrawButton(nItem,nSubItem,pDC,rect,pXLCD);
			*pResult = CDRF_SKIPDEFAULT;	// We've painted everything.
		}
		else
		{
							  ...
							  XLISTCTRLDATA *pXLCD)
{
	...
	if (str.IsEmpty())
	{
		...
		if (pXLCD[nSubItem].psa.GetSize()>0)
		{
			int index = 0;
...
BOOL CXListCtrl::DrawEdit(int nItem,
							  int nSubItem,
							  CDC *pDC,
							  COLORREF crText,
							  COLORREF crBkgnd,
...
// 	ArrowRect.left = ArrowRect.right - ArrowRect.Height();
// 	ArrowRect.DeflateRect(2, 2);
	//if(pXLCD->bEditActive)
	if(m_nClickItem == nItem && m_nClickSubItem == nSubItem)
	{
		if(m_pEdit && ::IsWindow(m_pEdit->m_hWnd))
		{
			...
			ScreenToClient(&rc);
// 			if(pXLCD[nSubItem].bEditAndCombox)
// 			{
// 				
// 				rc.left+=pXLCD[nSubItem].nWidthOfCombox+1;
// 			}
			TRACE(_T("row: %d   column:%d  Text:%s\n"),nItem,nSubItem);
			if(rc != rect)
			{
				m_pEdit->ShowWindow(SW_HIDE);
				if(!pXLCD[nSubItem].bEditAndCombox)
				{
					DrawText(nItem,nSubItem,pDC,RGB(0,0,0),RGB(255,255,255),rect,pXLCD);
				}
				m_nClickItem = m_nClickSubItem = -1;
	...
	else
	{
		...
		else
		{
			...
			else if(pXLCD[nSubItem].bEdit)
			{
				...
				CalculateCellRect(nSubItem,nItem,rcTemp);
				m_nClickItem = nItem;
				m_nClickSubItem = nSubItem;
				m_pEdit = ShowInPlaceEdit(nItem, nSubItem,rcTemp,str1);
				return;
			}
			else if(pXLCD[nSubItem].bEditAndCombox)
			{
				CalculateCellRect(nSubItem,nItem,rcTemp);
				rcCombox= CRect(rcTemp.left,rcTemp.top,rcTemp.left+pXLCD[nSubItem].nWidthOfCombox-1,rcTemp.bottom);
					rcEdit =CRect(rcTemp.left+pXLCD[nSubItem].nWidthOfCombox+1,rcTemp.top,rcTemp.right,rcTemp.bottom);
				str1=GetItemText(nItem,nSubItem);
				str1 = str1.Right(str1.GetLength()-str1.ReverseFind(TEXT(' '))-1);
				if(rcEdit.PtInRect(point))
				{
					m_nClickItem = nItem;
					m_nClickSubItem = nSubItem;
					CListCtrl::OnLButtonDown(nFlags, point);
					m_pEdit = ShowInPlaceEdit(nItem, nSubItem,rcEdit,str1);
				}
				else
				{
					m_nComboItem = nItem;
					...
					for(i=0;i<pXLCD[nSubItem].psa.GetSize();i++)
					{
						PopupMenu.AppendMenu(MF_ENABLED, IDM_COMBOX_MENU+i, pXLCD[nSubItem].psa.GetAt(i));
					}
					CPoint pt = rcCombox.TopLeft();
					ClientToScreen(&pt);
					PopupMenu.TrackPopupMenu(TPM_LEFTALIGN |TPM_RIGHTBUTTON ,pt.x,pt.y, this);
					return;
				}
			}
			else if(pXLCD[nSubItem].bButton)
			{
				m_nClickItem = nItem;
				m_nClickSubItem = nSubItem;
				pXLCD[nSubItem].bButtonActive = TRUE;
				SetTimer(5,200,NULL);
...
// 		pDC->DrawText(m_strNoItemText, -1, rc, DT_CENTER | DT_WORDBREAK | DT_NOPREFIX | DT_NOCLIP);
...
BOOL CXListCtrl::SetEdit(int nItem,int nSubItem,LPCTSTR lpszText,COLORREF crText, 
						COLORREF crBackground,BOOL bEdit /*= TRUE*/)
{
	...
	return SetItemText(nItem,nSubItem,lpszText,crText,crBackground);
}
///////////////////////////////////////////////////////////////////////////////
// DeleteProgress
							 ...
							 int nInitialComboSel)
{
	...
	if (bEnableCombo)
	{
		pXLCD[nSubItem].psa.RemoveAll();
		for(index=0;index<psa->GetSize();index++)
		{
			pXLCD[nSubItem].psa.Add((*psa)[index]);
		}
...
void CXListCtrl::OnTimer(UINT nIDEvent)
{
	...
	else if(nIDEvent == 5)
	{
		...
		{
			KillTimer(nIDEvent);
			XLISTCTRLDATA *pXLCD = (XLISTCTRLDATA *) CListCtrl::GetItemData(m_nClickItem);
			if(pXLCD && pXLCD[m_nClickSubItem].bButton)
			{
				pXLCD[m_nClickSubItem].bButtonActive = FALSE;
				CRect rc;
				GetSubItemRect(m_nClickItem,m_nClickSubItem,LVIR_BOUNDS,rc);
				if(rc.PtInRect(point))
				{
					...
					if (!pWnd)
						pWnd = GetOwner();
					if (pWnd && ::IsWindow(pWnd->m_hWnd))
						pWnd->PostMessage(WM_XLISTCTRL_BUTTON_CLICKED, 
						m_nClickItem, m_nClickSubItem);
				}
				
			}
			UpdateSubItem(m_nClickItem, m_nClickSubItem);
		}
	}
...
CInPlaceEdit* CXListCtrl::ShowInPlaceEdit(int iRowIndex,int iColumnIndex,const CRect & rcCell, CString& rstrCurSelection)
{
	...
	pInPlaceEdit->ShowEditCtrl(m_dwEditCtrlStyle, rcCell, this, 0, 
		iRowIndex, iColumnIndex,
		CString(""), rstrCurSelection);
	return pInPlaceEdit;
}

void CXListCtrl::CalculateCellRect(int iColumnIndex, int iRowIndex, CRect& robCellRect)
{
...
void CXListCtrl::ScrollToView(int iColumnIndex, /*int iOffSet, */CRect& robCellRect)
{
...
void CXListCtrl::OnEndlabeledit(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if(pXLCD[pDispInfo->item.iSubItem].bEditAndCombox)
	{
		strCell = pXLCD[pDispInfo->item.iSubItem].psa.GetAt(
			pXLCD[pDispInfo->item.iSubItem].nInitialComboSel);
		nLength =strCell.GetLength();
		ASSERT(nLength <0xff);
		nSubItem |= (nLength << 24);
		strCell += _T("  ");
		strCell += pDispInfo->item.pszText;
	}
	else
	{
		strCell = pDispInfo->item.pszText;
	}
	//TRACE(_T("row: %d column: %d Text: %s"),pDispInfo->item.iItem, pDispInfo->item.iSubItem, strCell);
	SetItemText(pDispInfo->item.iItem, pDispInfo->item.iSubItem, strCell);
	CWnd *pWnd = GetParent();
	if (!pWnd)
		pWnd = GetOwner();
	if (pWnd && ::IsWindow(pWnd->m_hWnd))
		pWnd->SendMessage(WM_EDIT_CHANGE,pDispInfo->item.iItem, nSubItem);
	*pResult = 0;
}
...
void CXListCtrl::SetListReadOnly(BOOL bReadOnly)
{
	m_bReadOnly = bReadOnly;
}

BOOL CXListCtrl::SetEditAndComboxInOneCell(int nItem, int nSubItem, CStringArray *psa,int nInitialComboSel,CString strEdit,
										   int nWidthOfCombox)
{
	ASSERT(nItem >= 0);
	ASSERT(nItem < GetItemCount());
	...
	pXLCD[nSubItem].Release();
	// update flag
	pXLCD[nSubItem].bEditAndCombox = TRUE;
	int index;
	pXLCD[nSubItem].psa.RemoveAll();
	for(index=0;index<psa->GetSize();index++)
	{
	...
	pXLCD[nSubItem].nWidthOfCombox = nWidthOfCombox;
	UpdateSubItem(nItem, nSubItem);
	return TRUE;
}
void CXListCtrl::OnComboxMenu(UINT nID)
{
	XLISTCTRLDATA *pXLCD = (XLISTCTRLDATA *) CListCtrl::GetItemData(m_nComboItem);
	if (!pXLCD)
	{
	...
	int index = nID - IDM_COMBOX_MENU;
	int nSubItem,nLength;
	if(index <0 || index >= pXLCD[m_nComboSubItem].psa.GetSize())
	{
		return;
	}
	CString str1,str2;
	str1 = GetItemText(m_nComboItem,m_nComboSubItem);
	str2 = pXLCD[m_nComboSubItem].psa.GetAt(pXLCD[m_nComboSubItem].nInitialComboSel);
	if(str1.Find(str2)!=0)
	{
		return;
	}
	str1 = str1.Right(str1.GetLength() - str2.GetLength());
	pXLCD[m_nComboSubItem].nInitialComboSel = index;
	str2 = pXLCD[m_nComboSubItem].psa.GetAt(index);
	nLength = str2.GetLength();
	ASSERT(nLength <0xff);
	nSubItem = (nLength << 24)|m_nComboSubItem;
	str2 += str1;
	SetItemText(m_nComboItem, m_nComboSubItem, str2);
	UpdateSubItem(m_nComboItem, m_nComboSubItem);
	...
	if (!pWnd)
		pWnd = GetOwner();
	if (pWnd && ::IsWindow(pWnd->m_hWnd))
		pWnd->PostMessage(WM_MENU_COMBO_SELECTION, 
									m_nComboItem, nSubItem);
}
...
void CXListCtrl::DeleteSubItem(int nItem, int nSubItem)
{
	ASSERT(nItem >= 0);
	ASSERT(nItem < GetItemCount());
	if ((nItem < 0) || nItem >= GetItemCount())
...
void CXListCtrl::DrawButton(int nItem, int nSubItem, CDC *pDC,CRect &rect, XLISTCTRLDATA *pXLCD)
{
	ASSERT(pDC);
	ASSERT(pXLCD);
	...
	rc.DeflateRect(2,0);
	rc.bottom -= 3;
	COLORREF clWindow=::GetSysColor(COLOR_BTNFACE);
	if(pXLCD[nSubItem].bButtonActive)
	{
		rc.top += 2;
		rc.bottom += 2;
	}
	pDC->FillSolidRect(rc,clWindow);
	...
	pDC->DrawText(pXLCD[nSubItem].strButtonText, &rc, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
	pDC->SelectObject(pOldFont);
	pDC->SetTextColor(oldTextColor);
}

BOOL CXListCtrl::SetButtom(int nItem, int nSubItem, LPCTSTR lpszText)
{
	ASSERT(nItem >= 0);
	ASSERT(nItem < GetItemCount());
	if ((nItem < 0) || nItem >= GetItemCount())
	...
	pXLCD[nSubItem].strButtonText = lpszText;
	UpdateSubItem(nItem, nSubItem);
	return TRUE;
}
...
void CXListCtrl::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
	// TODO: Add your message handler code here and/or call default
// 	CRect rc;
// 	if(m_nClickItem >= 0 && m_nClickSubItem >= 0)
// 	{
// 		GetSubItemRect(m_nClickItem,m_nClickSubItem,LVIR_BOUNDS,rc);	
// 		::RedrawWindow(m_hWnd,&rc,NULL,RDW_UPDATENOW|RDW_INVALIDATE);
// 	}
	if(m_pEdit && IsWindow(m_pEdit->m_hWnd))
	{
		m_pEdit->ShowWindow(SW_HIDE);
