...
CMultiTree::CMultiTree()
{
	...
	m_bBandLabel = FALSE;
	m_bEmulated = FALSE;
	m_bHasCannotMultiSelectItem = TRUE;
}
...
void CMultiTree::SelectRange(HTREEITEM hFirst, HTREEITEM hLast, BOOL bOnly /*= TRUE*/)
{
	...
	HTREEITEM hItem = GetRootItem();
	//寻找从根开始，可见的第一个被选中的HTREEITEM，当然，它只能是hFirst和hLast中的一个
	while (hItem) 
	{
...
/********************************************************************************
功能：点击树节点的响应函数
输入：bLeft：左击true，右击：false；nFlags：？？？？，point：点击位置
返回：无
********************************************************************************/
...
/********************************************************************************
功能：选中节点前
输入：hItem：树节点 bLeft：左击true，右击：false；nFlags：？？？？
返回：无
********************************************************************************/
void CMultiTree::DoPreSelection(HTREEITEM hItem, BOOL bLeft, UINT nFlags)
{
	//dwIndexOfMultiSelectItem 以1 为基数
	DWORD dwIndexOfMultiSelectItem = CTreeCtrl::GetItemData(hItem);
	if (bLeft)
	{
		//if shift key down, select immediately
		if(m_bHasCannotMultiSelectItem || (dwIndexOfMultiSelectItem == 0))
		{
			NotSelectAnyItem();
			//让m_bHasCannotMultiSelectItem只能为0或1
			m_bHasCannotMultiSelectItem = !dwIndexOfMultiSelectItem;
		...
		else if (_bShift) 
		{
			...
			m_bHasCannotMultiSelectItem = FALSE;
		}
		else
		{
			if (!_bCtrl) 
	...
	//right mouse
	if (nFlags & (MK_CONTROL|MK_SHIFT))  //右击并同时按下CONTROL或SHIFT
	{
		if (!_bShift)
			m_hSelect = hItem;
...
/********************************************************************************
功能：树节点的展开
输入：hParent为父节点， bSelect： bRecurse
返回：
********************************************************************************/
...
返回：
********************************************************************************/
BOOL CMultiTree::SelectChildren(HTREEITEM hParent, BOOL bSelect /*= TRUE*/, 
								BOOL bRecurse /*= TRUE*/)
{
	...
	while (hItem)
	{
		...
		bFocusWasInHere |= (nState & TVIS_FOCUSED);      // bFocusWasInHere | (nState & TVIS_FOCUSED)；bFocusWasInHere本身为FALSE即0，0或任何值都为其本身
		if (bRecurse && (nState & TVIS_EXPANDED))
			bFocusWasInHere |= SelectChildren(hItem, bSelect, bRecurse);
		hItem = GetNextSiblingItem(hItem);               //获得兄弟节点
	}
...
HTREEITEM CMultiTree::InsertItem(const CString & strItemName,int nImage, int nSelectedImage,
		HTREEITEM hParent/* = TVI_ROOT*/,HTREEITEM hInsertAfter/* = TVI_LAST*/,DWORD dwItemData/*=0*/)
{
	HTREEITEM re = CMultiTree_BASE::InsertItem(strItemName,nImage,nSelectedImage,hParent,hInsertAfter);
	CMultiTree_BASE::SetItemData(re,(DWORD)dwItemData);
	return re;
}
...
返回：
********************************************************************************/
void CMultiTree::NotSelectAnyItem()
{
	HTREEITEM hItem =GetFirstSelectedItem();
	while(hItem!=NULL)
	{
