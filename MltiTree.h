...
// Implementation
public:
	void NotSelectAnyItem();
	//dwItemData,当允许树结点多选时有效，
	//允许树结点多选时:dwItemData表示从根开始，它是第几个允许多选的项，以1为基数
	HTREEITEM InsertItem(const CString & strItemName,int nImage, int nSelectedImage,
...
protected:
	BOOL m_bHasCannotMultiSelectItem;   //不是多选为1，反之为0
	//{{AFX_MSG(CMultiTree)
